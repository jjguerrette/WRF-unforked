...
#         mpistring="$MPICALL $DEBUGSTR -np $nproc_max $EXECUTABLE"
   ...
   if [ $WRF_CHEM -gt 0 ]; then
      if [ $(ls "$CWD"/SURFACE_Hx_y* | wc -l) -eq 0 ]; then echo "ERROR: Missing SURFACE_Hx_y files"; echo 14; exit 14; fi

      if [ $(ls "$CWD"/AIRCRAFT_Hx_y* | wc -l) -eq 0 ]; then echo "ERROR: Missing AIRCRAFT_Hx_y files"; echo 15; exit 15; fi
